...
%>
        ...
        response.setCharacterEncoding("UTF-8");
            try{
                ResultSet rs;
                String sql = "SELECT * FROM ssd WHERE cod_prod = ?";
                pstm = con.prepareStatement(sql);
                pstm.setInt(1, Integer.parseInt(request.getParameter("cod_prod")));
                rs = pstm.executeQuery();            
                while (rs.next()) {
                %>
                    <form id="form1" action="../gravar/gr_alt_ssd.jsp" method="post">
                        <table>
                            ...
                            <tr><td> <label class="frm"> Velocidade Leitura: </label> </td><td><input type="text" name="vel_leitura" value="<%= rs.getString("vel_leitura") %>" /> </td></tr>
                            <tr><td> <label class="frm"> Velocidade Escrita: </label> </td><td><input type="text" name="vel_escrita" value="<%= rs.getString("vel_escrita") %>" /> </td></tr>
