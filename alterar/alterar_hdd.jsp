...
%>        
        ...
        response.setCharacterEncoding("UTF-8");
            try{
                ResultSet rs;
                String sql = "SELECT * FROM hdd WHERE cod_prod = ?";
                pstm = con.prepareStatement(sql);
                pstm.setInt(1, Integer.parseInt(request.getParameter("cod_prod")));
                rs = pstm.executeQuery();            
                while (rs.next()) {
                %>
                    <form id="form1" action="../gravar/gr_alt_hdd.jsp" method="post">
                        <table>
                            ...
                            <tr><td> <label class="frm"> RPM: </label> </td><td><input type="text" name="rpm" value="<%= rs.getString("rpm") %>" /> </td></tr>
