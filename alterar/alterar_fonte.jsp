...
%>        
        ...
        response.setCharacterEncoding("UTF-8");
            try{
                ResultSet rs;
                String sql = "SELECT * FROM fonte WHERE cod_prod = ?";
                pstm = con.prepareStatement(sql);
                pstm.setInt(1, Integer.parseInt(request.getParameter("cod_prod")));
                rs = pstm.executeQuery();            
                while (rs.next()) {
                %>
                    <form id="form1" action="../gravar/gr_alt_fonte.jsp" method="post">
                        <table>
                            ...
                            <tr><td> <label class="frm"> Potencia: </label> </td><td><input type="text" name="potencia" value="<%= rs.getString("potencia") %>" /> </td></tr>
                            <tr><td> <label class="frm"> Certificação </label> </td><td><input type="text" name="certificacao" value="<%= rs.getString("certificacao") %>" /> </td></tr>
