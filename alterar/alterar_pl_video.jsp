...
%>      
        ...
        response.setCharacterEncoding("UTF-8");
            try{
                ResultSet rs;
                String sql = "SELECT * FROM placa_video WHERE cod_prod = ?";
                pstm = con.prepareStatement(sql);
                pstm.setInt(1, Integer.parseInt(request.getParameter("cod_prod")));
                rs = pstm.executeQuery();            
                while (rs.next()) {
                %>
                    <form id="form1" action="../gravar/gr_alt_pl_video.jsp" method="post">
                        <table>
                            ...
                            <tr><td> <label class="frm"> Clock: </label> </td><td><input type="text" name="clock" value="<%= rs.getString("clock") %>" /> </td></tr>
                            <tr><td> <label class="frm"> Pinos: </label> </td><td><input type="text" name="pinos" value="<%= rs.getString("pinos") %>" /> </td></tr>
